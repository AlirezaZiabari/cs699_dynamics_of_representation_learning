...
from utils.evaluations import get_loss_value, get_loss_value_for_adversarial_images
    ...
    with tqdm(total=x_num * y_num) as pbar:
        for idx_x, x in enumerate(xcoordinates):
            for idx_y, y in enumerate(ycoordinates):
                set_weights_by_direction(model, x, y, direction1, direction2, pretrained_weights, skip_bn_bias=args.skip_bn_bias)
                
                if args.use_attack:
                    losses[idx_x, idx_y], accuracies[idx_x, idx_y] = get_loss_value_for_adversarial_images(model, adversarial_image_path, args.device)
                else:
