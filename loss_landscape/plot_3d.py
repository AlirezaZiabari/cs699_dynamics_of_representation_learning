...
def animate(i):
    # this function is an internal auxillary function for plotting gifs
...
if __name__ == '__main__':
    ...
    if args.surface_file:
        ...
        losses_within = losses[((args.x_uplim > xcoords) & (args.x_lowlim < xcoords)), :][:, (args.y_uplim > ycoords) & (args.y_lowlim < ycoords)]
        ...
        fig.savefig(
            f"{args.result_folder}/{args.plot_prefix}_surface_2d_contour_level{loss_level_diff:.0e}_zlim{args.zlim}", dpi=300,
            bbox_inches='tight'
        )
    ...
    #     loss_interp_fn = interpolate.interp2d(xcoords_within, ycoords_within, np.transpose(losses_within), kind='quintic')
        
    #     xcoords = data["xcoordinates"]
    #     ycoords = data["ycoordinates"]
    #     xcoords_within_tj = xcoords[(args.x_uplim > xcoords) & (args.x_lowlim < xcoords) & (args.y_uplim > ycoords) & (args.y_lowlim < ycoords)]
    #     ycoords_within_tj = ycoords[(args.x_uplim > xcoords) & (args.x_lowlim < xcoords) & (args.y_uplim > ycoords) & (args.y_lowlim < ycoords)]
    
    #     losses_within_tj = loss_interp_fn(xcoords_within_tj, ycoords_within_tj)
    #     losses_within_tj[losses_within_tj>args.zlim] = args.zlim
        
    #     ax.plot(xcoords_within_tj, ycoords_within_tj, np.diag(losses_within_tj), linewidth=0.5, color='black')
    #     TJ = ax.scatter(xcoords_within_tj, ycoords_within_tj, np.diag(losses_within_tj), marker='.', color='black')
    #     ax.tick_params('y', labelsize='x-large')
    #     ax.tick_params('x', labelsize='x-large')
    #     ax.set_zlim(0, args.zlim)
        
    #     fig.savefig(
    #         f"{args.result_folder}/{args.plot_prefix}_trajectory+contour_2d_level{loss_level_diff:.0e}_zlim{args.zlim}", dpi=300,
    #         bbox_inches='tight'
    #     )
    #     pyplot.show()
